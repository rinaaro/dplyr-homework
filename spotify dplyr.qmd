...
```{r med_age_table}
knitr::kable(med_age|>
  pivot_wider(names_from = Gender, values_from = `median(\`Age at enrollment\`)`), caption = "Median age at enrollment by marital status and gender")
```
...
```{r curr_mean_table}
knitr::kable(curr_mean|>
  pivot_longer(-Target, names_to = "Units") |>
  pivot_wider(names_from = Target),  caption = "Mean curricular units per target", digits = 2)
```